...
def get_car_performance(df, team):
    ...
    team_avg_grid = (
        team_df["GridPosition"].mean()
    )

    podiums = (
        (
            team_df["FinishPosition"] <= 3
        ) &
        (
            team_df["DNF"] == 0
        )
...
def build_feature_row(
    ...
    """
    driver_stats = get_driver_stats(
        historical_df,
        driver
    )

    circuit_stats = get_circuit_history(
        historical_df,
        driver,
        circuit
    )

    wet_stats = get_driver_wet_skill(
        historical_df,
        driver
    )

    team_stats = get_car_performance(
        historical_df,
        team
    )
    recent3 = get_recent_form(
        historical_df,
        driver,
        3
    )

    recent5 = get_recent_form(
        historical_df,
        driver,
        5
    )
