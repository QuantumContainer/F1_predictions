...
def build_season_data(year, force_refresh=False):
    ...
    if os.path.exists(filepath) and not force_refresh:
        ...
        print(f"Loading cached season data for {year}")
        return pd.read_csv(filepath)

    # Build Season From FastF1
    print(f"Building season data for {year}")
    ...
    for _, event in schedule.iterrows():
        ...
        try:
            ...
            race_df.rename(
                columns={"Abbreviation": "Driver","Position": "FinishPosition"},inplace=True)

            weather = get_weather_features(session)
